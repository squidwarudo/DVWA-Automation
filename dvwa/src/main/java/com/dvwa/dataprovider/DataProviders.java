...
import com.dvwa.utility.NewExcelLibrary;
...
public class DataProviders {
	...
	//Class --> AccountCreationPage  Test Case--> verifyCreateAccountPageTest	
		@DataProvider(name = "addPostalArea")
		public Object[][] getEmail() {
			// Totals rows count
			int rows = obj.getRowCount("AddPostalArea");
			// Total Columns
			int column = obj.getColumnCount("AddPostalArea");
			int actRows = rows - 1;
			...
			for (int i = 0; i < actRows; i++) {
				for (int j = 0; j < column; j++) {
					data[i][j] = obj.getCellData("AddPostalArea", j, i + 2);
				}
			}
			return data;
		}
	...
	//Class --> AddToCartPageTest, EndToEndTest,  Test Case--> addToCartTest, endToEndTest	
		...
		public Object[][] getProduct() {
			// Totals rows count
			int rows = obj.getRowCount("InvalidCredentials");
			// Total Columns
			int column = obj.getColumnCount("InvalidCredentials");
			int actRows = rows - 1;
			...
			for (int i = 0; i < actRows; i++) {
				for (int j = 0; j < column; j++) {
					data[i][j] = obj.getCellData("InvalidCredentials", j, i + 2);
				}
			}
			return data;
		}
